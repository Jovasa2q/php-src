...
{
	if (UNEXPECTED(class_type->ce_flags & (ZEND_ACC_INTERFACE|ZEND_ACC_TRAIT|ZEND_ACC_IMPLICIT_ABSTRACT_CLASS|ZEND_ACC_EXPLICIT_ABSTRACT_CLASS|ZEND_ACC_ENUM|ZEND_ACC_TYPE_ALIAS))) {
		...
		} else if (class_type->ce_flags & ZEND_ACC_TYPE_ALIAS) {
			zend_throw_error(NULL, "Cannot instantiate type alias %s", ZSTR_VAL(class_type->name));
		} else {
